...
function ra(src, from, to) { return src.split(from).join(to); }
...
html = ra(html, '<option value="midnight_classic">????</option>', '<option value="midnight_classic">\u591C\u5E55\u7ECF\u5178</option>');
html = ra(html, '<option value="ice_deep">????</option>',         '<option value="ice_deep">\u6DF1\u7A7A\u51B0\u84DD</option>');
html = ra(html, '<option value="amber_dawn">????</option>',       '<option value="amber_dawn">\u6668\u5149\u7425\u73C0</option>');
html = ra(html, '<option value="sky_light">????</option>',        '<option value="sky_light">\u6674\u7A7A\u6D45\u84DD</option>');
html = ra(html, '<option value="cherry_blossom">????</option>',   '<option value="cherry_blossom">\u6A31\u96FE\u7C89\u663C</option>');
html = ra(html, '<option value="arctic_mint">????</option>',      '<option value="arctic_mint">\u8584\u8377\u6781\u5149</option>');
...
html = ra(html, '<option value="all" selected>??</option>',  '<option value="all" selected>\u5168\u90E8</option>');
html = ra(html, '<option value="image">???</option>',        '<option value="image">\u4EC5\u56FE\u7247</option>');
html = ra(html, '<option value="video">???</option>',        '<option value="video">\u4EC5\u89C6\u9891</option>');

// 排序（出现两次：toolbar + mobile sheet）
html = ra(html, '<option value="date_taken|DESC">????</option>', '<option value="date_taken|DESC">\u6700\u65B0\u62CD\u6444</option>');
html = ra(html, '<option value="date_taken|ASC">????</option>',  '<option value="date_taken|ASC">\u6700\u65E9\u62CD\u6444</option>');
html = ra(html, '<option value="file_name|ASC">??? A-Z</option>',  '<option value="file_name|ASC">\u6587\u4EF6\u540D A-Z</option>');
html = ra(html, '<option value="file_name|DESC">??? Z-A</option>', '<option value="file_name|DESC">\u6587\u4EF6\u540D Z-A</option>');
html = ra(html, '<option value="file_size|DESC">??????</option>', '<option value="file_size|DESC">\u4F53\u79EF\u4ECE\u5927\u5230\u5C0F</option>');
html = ra(html, '<option value="file_size|ASC">??????</option>',  '<option value="file_size|ASC">\u4F53\u79EF\u4ECE\u5C0F\u5230\u5927</option>');
...
html = ra(html, '<option value="3" selected>3 \u003F</option>', '<option value="3" selected>3 \u79D2</option>');
