      ...
      function getMediaAspectRatioValue(photo) {
        var row = photo || {};
        var w = parseFloat(
          row.width || row.pixel_width || row.file_width || row.media_width || 0,
        );
        var h = parseFloat(
          row.height || row.pixel_height || row.file_height || row.media_height || 0,
        );
        if (!(w > 0 && h > 0)) return '';
        var r = w / h;
        ...
        try {
          ...
          if (parsed && typeof parsed === 'object') {
            ...
            state.subtitlePosition =
              p === 'top' || p === 'middle' || p === 'bottom' ? p : 'bottom';
          }
        } catch (e) {}
      }
      ...
      function loadWebVideoForPreview(photo, video, index, requestSeq, pauseAfterSwitch) {
        ...
        $('#previewInfo').textContent = buildPreviewInfoText(photo, index) + ' \u00B7 \u51C6\u5907\u64AD\u653E...';
        ...
        fetch('/api/video-playback?id=' + photo.id, { credentials: 'same-origin' })
          ...
          .then(function (data) {
            ...
            if (data.error === 'not_found') {
              stopLoadingForVideo();
              $('#previewInfo').textContent = buildPreviewInfoText(photo, index) + ' \u00B7 \u5A92\u4F53\u4E0D\u5B58\u5728';
              return;
            }
            ...
            if (!data.ready) {
              stopLoadingForVideo();
              $('#previewInfo').textContent = buildPreviewInfoText(photo, index) + ' \u00B7 \u65E0\u6CD5\u64AD\u653E';
              return;
            }
      ...
      function showInstallGuide() {
        if (isStandaloneMode()) {
          window.alert('\u5DF2\u5B89\u88C5\u5230\u684C\u9762\uFF0C\u53EF\u76F4\u63A5\u5728\u624B\u673A\u684C\u9762\u6253\u5F00\u3002');
          return;
        }
        ...
        if (isIosSafari()) {
          window.alert('\u8BF7\u70B9\u51FB Safari \u5E95\u90E8\u201C\u5206\u4EAB\u201D\u6309\u94AE\uFF0C\u7136\u540E\u9009\u62E9\u201C\u6DFB\u52A0\u5230\u4E3B\u5C4F\u5E55\u201D\u3002');
          return;
        }
        window.alert('\u8BF7\u70B9\u51FB\u6D4F\u89C8\u5668\u53F3\u4E0A\u89D2\u83DC\u5355\uFF0C\u9009\u62E9\u201C\u5B89\u88C5\u5E94\u7528\u201D\u6216\u201C\u6DFB\u52A0\u5230\u4E3B\u5C4F\u5E55\u201D\u3002');
      }
      ...
      function updateSidebarActive() {
        ...
        if (state.currentView === 'folder' && state.currentPath) {
          var currentEl = document.querySelector('#sidebarContent .folder-item[data-folder-path="' + CSS.escape(state.currentPath) + '"]');
          if (currentEl) {
      ...
      function createGridFallbackPlaceholder(card) {
        ...
        ph.innerHTML = '<div class="ext">\u26A0\uFE0F</div><div>\u7F29\u7565\u56FE\u52A0\u8F7D\u5931\u8D25</div>';
        return ph;
      }
        ...
        try {
            ...
            case 'folder':
              ...
              try {
                var subfolderResp = await apiGet(
                  '/api/immediate-subfolder-covers?parentPath=' + encodeURIComponent(state.currentPath) +
                  (state.mediaFilter && state.mediaFilter !== 'all'
                    ? '&mediaType=' + encodeURIComponent(state.mediaFilter)
                    : '')
                );
                state.currentSubfolderCovers = Array.isArray(subfolderResp) ? subfolderResp :
          ...
          if (state.currentView === 'folder') {
            ...
            if (subfolderCount > 0 && scopedTotal > 0) {
              statsText = formatNumber(subfolderCount) + ' 个子文件夹 | ' +
                formatNumber(scopedTotal) + ' 张照片 | 视频 ' +
                formatNumber(scopedVideoCount) + ' 条';
            } else if (subfolderCount > 0) {
              statsText = formatNumber(subfolderCount) + ' 个子文件夹 | 0 张照片';
            } else {
              statsText = scopedTotal > 0
      ...
      function renderPhotoGrid(photos) {
        var hasSubfolders = state.currentView === 'folder' && state.currentSubfolderCovers && state.currentSubfolderCovers.length > 0;
        ...
        var html =
          '<div class="grid' + (isMasonryAspect ? ' grid--masonry' : '') + '" data-use-media-ratio="' + (useMediaRatio ? '1' : '0') + '">';
        ...
        for (var i = 0; i < photos.length; i++) {
          var photo = photos[i];
          var isVideo = isWebVideoFileType(photo.file_type) || String(photo.media_type || '').toLowerCase() === 'video';
      ...
      function buildPreviewRandomBatchQuery(excludeIds) {
        ...
        var ex = Array.isArray(excludeIds)
          ...
          : [];
        if (ex.length)
          params += '&excludeIds=' + encodeURIComponent(ex.join(','));
        return params;
      }
      ...
      async function loadPreviewAdjacentPage(pageDir, dir) {
        ...
        if (pageDir > 0) {
          nextPage =
            state.previewPageStart + Math.ceil(state.previewPhotos.length / state.pageSize);
        } else {
      ...
      function installDevErrorOverlay() {
        ...
        window.addEventListener('unhandledrejection', function (ev) {
          ...
          var detail =
            reason && reason.stack ? String(reason.stack) : typeof reason === 'string' ? reason : JSON.stringify(reason);
