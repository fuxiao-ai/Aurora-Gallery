...
- **Bundle ID**（electron-builder `appId`）：`com.foredawn.aurora-gallery`

**当前发布版本**：`1.0.1`（与根目录 `package.json` 的 `version` 字段一致；发版前请 bump 版本并同步「关于」等文案。）
...
常见输出（文件名中的版本号与 `package.json` 的 `version` 一致，例如当前为 `1.0.1`）：
